...
class STAttentionBlock(nn.Module):
    def __init__(self, in_channels, out_channels, inter_channels, num_subset=8, num_node=25, num_frame=32,
        ...
        # print('num_subset:', self.num_subset)
        ...
        if use_spatial_att:
            ...
            if att_s:
                ...
                if self.num_node == 18: # Kinetics series
                    from graph2.kinetics import AdjMatrixGraph
                elif self.num_node == 25: # NTU RGB+D series
                    from graph2.ntu_rgb_d import AdjMatrixGraph
    ...
    def forward(self, x):
        ...
        if self.use_spatial_att:
            ...
            if self.att_s:
                ...
                # print('224 y shape:', y.size())
                # print('226 inchannels:', self.in_channels)
                # print('diff size:', torch.einsum('nctu,uv->nctv',y, self.graph_a).repeat(1,1,1,self.window_size).size())
                # print('num_subset:', self.num_subset)
                # print('result:',self.in_channels//self.num_subset)
...
if __name__ == '__main__':
    ...
    net.cuda()
    ske = torch.rand([2, 3, 32, 25, 2], device=cuda0)  # .cuda()
    print(net(ske).shape)
