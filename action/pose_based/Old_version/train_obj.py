...
def run(args):
    ...
    train_dataset = Dataset(args.dataset_path, db_filename=args.db_filename, train_filename=args.train_filename,
                        ...
                        arch=args.arch, obj_path=args.obj_path)
    ...
    test_dataset = Dataset(args.dataset_path, db_filename=args.db_filename, train_filename=args.train_filename,
                           ...
                           pose_path=args.pose_relative_path, arch=args.arch)
    ...
    model = ObjectNet.Model(num_class=num_classes, num_point=18, num_person=1, 
    ...
    print('Model type:{}, number of parameters:{}M'.format(arch, round(model_total_params, 2)))
    ...
    while steps < max_steps:
        ...
        for train_batchind, data in enumerate(tqdm(train_dataloader)):
            ...
            print("Loaded one batch data time:",time.time()-start)
            model.train(True)
            num_iter += 1
            # get the inputs
            ...
            inputs = Variable(object_data.cuda(), requires_grad=True)
        ...
        with torch.no_grad():
            for test_batchind, data in enumerate(tqdm(test_dataloader)):
                ...
                inputs = Variable(object_data.cuda())
                labels = Variable(labels.cuda())
                labels = torch.argmax(labels, dim=1)
