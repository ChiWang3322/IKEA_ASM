...
def run(args):
    ...
    if arch == 'AGCN':
        skeleton_stream =  agcn.Model(num_class=num_classes, **model_args)
    elif arch == 'EGCN':
        #B5
        ...
        skeleton_stream = EGCN(**model_args, **kargs)
    else:
        raise ValueError("Unsupported architecture: please select HCN | ST_GCN | AGCN | STGAN")
    obj_stream =  ObjectNet.Model(num_class=num_classes, num_point=18, num_person=1, 
                    graph='graph.kinetics.Graph', graph_args={'labeling_mode':'spatial'}, in_channels=5)
    if args.with_obj:
        model = ObjectNet.Combined_model(skeleton_model = skeleton_stream, obj_model=obj_stream)   
    else:
        model = skeleton_stream


    if args.refine:
        if args.refine_epoch == 0:
            # raise ValueError("You set the refine epoch to 0. No need to refine, just retrain.")
            print('Refining model')
            refine_model_path = os.path.join(args.logdir, 'best_classifier.pth')
            checkpoint = torch.load(refine_model_path)
    ...
    num_steps_per_update = args.steps_per_update  # accum gradient - try to have number of examples per update match original code 8*5*4
    eval_steps  = 5
    # train it
    n_examples = 0
    ...
    while steps < max_steps:
        ...
        for train_batchind, data in enumerate(tqdm(train_dataloader)):
            ...
            if arch == 'EGCN':
                # Joint, motion and bone
                skeleton_data = batch_multi_input(skeleton_data)
            # wrap them in Variable
            # N x B x C x T x V x M
            skeleton_data = Variable(skeleton_data.cuda(), requires_grad=True)
            # print("size skeleton:", skeleton_data.size())
            # N x C x T x V x M
            if args.with_obj:
                
                object_data = Variable(object_data.cuda(), requires_grad=True)
                inputs = (skeleton_data, object_data)
            # print("object size:", object_data.size())
