...
parser.add_argument('--dataset_path', type=str, default='/home/chiwang/Python/IKEA_Benchmark/IKEA_ASM_Dataset/dataset/ANU_ikea_dataset/',
...
#    print(single_label_per_frame.size())
    # print(torch.tensor(gt_labels[vid_idx]).size())
    # print(torch.tensor(logits[vid_idx]).size())
...
f1_score_micro = sklearn.metrics.f1_score(np.concatenate(gt_single_labels).ravel(), np.concatenate(pred_labels).ravel(), average='micro')
print('f1_score micro:',f1_score_micro)
f1_score_macro = sklearn.metrics.f1_score(np.concatenate(gt_single_labels).ravel(), np.concatenate(pred_labels).ravel(), average='macro')
print('f1_score macro:',f1_score_macro)
class_names = utils.squeeze_class_names(dataset.action_list)
# print("gt_single_labels:",gt_single_labels)
